...
class SurfaceVesselEnv(gym.Env):
    ...
    def __init__(self, max_steps=10000,show_viewport=True):
        super().__init__()
    ...
    def _get_obs(self, sensors):
        ...
        #Here we add the lidar data processing code
        # 1. Initialize an array of 64 beams set to Max Range (70.0)
        lidar_distances = np.full(64, 70.0, dtype=np.float32)
        
        # 2. Process LiDAR hits (Bulletproof version)
        if "Lidar" in sensors and sensors["Lidar"].size > 0:
            lidar_data = sensors["Lidar"] 
            
            # If HoloOcean returns a flat 1D array, reshape it into groups of 3 (X,Y,Z) or 4 (X,Y,Z,Intensity)
            ...
            hit_distances = np.linalg.norm(lidar_data[:, :3], axis=1)
            angles = np.arctan2(lidar_data[:, 1], lidar_data[:, 0]) # Radians
            angles_deg = (np.degrees(angles) + 360) % 360           # Convert to [0, 360)
            
            # Map angle to beam index [0 to 63]
            beam_indices = np.floor(angles_deg / 5.625).astype(int)
            beam_indices = np.clip(beam_indices, 0, 63) # Prevent index out of bounds
            
            for idx, beam_dist in zip(beam_indices, hit_distances):
                if beam_dist < lidar_distances[idx]:
                    lidar_distances[idx] = beam_dist
                    
        # 3. Normalize the 64 distances to [0, 1]
        ...
        return np.concatenate([
            [dist, float(angle_yaw), speed, vel_xy[0], vel_xy[1], yaw_rate],
            normalized_lidar
        ], dtype=np.float32)
